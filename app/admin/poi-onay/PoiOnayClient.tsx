...
// Bileşeni dışarıda tanımla — inline anti-pattern'den kaçın (CLAUDE.md §9)
  ...
  return (
    <div style={{ borderTop: `1px solid ${C.border}`, marginTop: 12, paddingTop: 14 }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10, marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
            ...
            />
            <span style={{ color: C.muted, fontSize: '0.82rem', fontWeight: 600 }}>🆘 Acil/SOS noktası</span>
          </label>
        </div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button
          onClick={kaydet}
          disabled={kayitYukleniyor || !form.name.trim()}
          style={{
            background: C.greenDark, color: C.green,
            border: `1px solid ${C.greenBg}`, borderRadius: 6,
            padding: '6px 18px', fontSize: '0.82rem', fontWeight: 700,
            cursor: kayitYukleniyor || !form.name.trim() ? 'not-allowed' : 'pointer',
            opacity: kayitYukleniyor || !form.name.trim() ? 0.5 : 1,
          }}
        >
  ...
  async function icerikGuncelle(id: string, fields: Partial<Poi>) {
    ...
    try {
      ...
      if (d.success) {
        ...
        setHata(d.error || 'Güncelleme başarısız.');
      }
    } catch {
      setHata('Bağlantı hatası.');
    } finally {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {pois.map(poi => (
            <div
              key={poi.id}
              style={{
                background: C.surface, border: `1px solid ${duzenleId === poi.id ? C.blue : C.border}`,
                borderRadius: 8, padding: '14px 16px',
                transition: 'border-color 0.15s',
              }}
            >
              {/* Üst satır: bilgiler + butonlar */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
                ...
                {/* Düzenle butonu — tüm sekmelerde göster */}
                <button
                  onClick={() => setDuzenleId(duzenleId === poi.id ? null : poi.id)}
                  style={{
                    background: duzenleId === poi.id ? C.blueBg : 'transparent',
              ...
              {/* Düzenleme formu — satır genişler */}
              {duzenleId === poi.id && (
                <DuzenleForm
                  poi={poi}
