...
export default function Aksiyonlar({ ilanId }: { ilanId: string }) {
  ...
  async function sikayetGonder() {
    ...
    // Mevcut complaints listesini çek
    ...
    const yeniSikayet = {
      neden: secim,
      aciklama: secim === 'Diğer' ? aciklama : '',
      tarih: new Date().toISOString(),
    };

    await supabase.from('listings').update({
      complaints: [...mevcutComplaints, yeniSikayet],
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={paylas}
          style={{
            ...
            background: kopyalandi ? '#14532d' : '#1f2937',
            border: `1px solid ${kopyalandi ? '#166534' : '#374151'}`,
            color: kopyalandi ? '#22c55e' : '#9ca3af',
            borderRadius: 8, padding: '9px 18px',
            fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer',
            transition: 'all 0.2s'
          }}>
          {kopyalandi ? (
            <><span>✅</span> Bağlantı kopyalandı</>
          ) : (
            <><span style={{ fontSize: '1rem' }}>↗</span> İlanı Paylaş</>
          )}
        </button>
        ...
        {!gonderildi ? (
          <button onClick={() => setSikayetAcik(!sikayetAcik)}
            style={{
              ...
              color: sikayetAcik ? '#ef4444' : '#4b5563',
        ...
        <div style={{
            ...
            {SIKAYET_SECENEKLERI.map(s => (
              <label key={s}
                style={{
                  ...
                  background: secim === s ? '#1a1a2e' : 'transparent',
                  border: `1px solid ${secim === s ? '#4f46e5' : '#1f2937'}`,
                  transition: 'all 0.15s'
                }}>
                <div style={{
                  width: 16, height: 16, borderRadius: '50%', flexShrink: 0,
                  border: `2px solid ${secim === s ? '#4f46e5' : '#374151'}`,
                  background: secim === s ? '#4f46e5' : 'transparent',
                  display: 'flex', alignItems: 'center', justifyContent: 'center'
                }}>
                  {secim === s && <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#fff' }} />}
                </div>
                <input type="radio" name="sikayet" value={s} checked={secim === s}
                  onChange={() => setSecim(s)} style={{ display: 'none' }} />
          ...
          {secim === 'Diğer' && (
            <textarea
              ...
              rows={3}
              style={{
                width: '100%', background: '#161b22', color: '#e2e8f0',
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={sikayetGonder} disabled={!secim}
              style={{
                background: secim ? '#dc2626' : '#374151',
                color: '#fff', border: 'none', borderRadius: 6,
                padding: '8px 20px', fontSize: '0.85rem', fontWeight: 700,
                cursor: secim ? 'pointer' : 'not-allowed',
