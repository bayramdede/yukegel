...
export default function Moderator() {
  ...
  const filtrelenmis = ilanlar.filter(ilan => {
    ...
    if (filtreTelefon) {
      const tel = filtreTelefon.replace(/\D/g, '');
      if (!(ilan.contact_phone || '').replace(/\D/g, '').includes(tel)) return false;
    }
  ...
  // Yeşil / sarı / kırmızı sayıları (pending için)
  ...
  function stopEkle() {
    setDuzenleData({
      ...duzenleData,
      stops: [...(duzenleData.stops || []), { id: null, city: 'İstanbul', district: '', weight_ton: '', pallet_count: '', vehicle_count: 1, cargo_type: '', notes: '' }]
    });
  }

  function stopSil(idx: number) {
    setDuzenleData({ ...duzenleData, stops: duzenleData.stops.filter((_: any, i: number) => i !== idx) });
  }
  ...
  async function aliasOgren(data: any) {
    ...
    if (data.origin_district && data.origin_city) {
      await supabase.from('aliases').upsert({ alias: norm(data.origin_district), normalized: data.origin_city, district: data.origin_district, type: 'city', is_active: true, priority: 90 }, { onConflict: 'alias' });
    }
    for (const stop of (data.stops || [])) {
      if (stop.district && stop.city) {
        await supabase.from('aliases').upsert({ alias: norm(stop.district), normalized: stop.city, district: stop.district, type: 'city', is_active: true, priority: 90 }, { onConflict: 'alias' });
      }
    }
    for (const vt of (data.vehicle_type || [])) {
      if (vt) await supabase.from('aliases').upsert({ alias: norm(vt), normalized: vt, type: 'vehicle', is_active: true, priority: 80 }, { onConflict: 'alias' });
    }
    for (const bt of (data.body_type || [])) {
      if (bt) await supabase.from('aliases').upsert({ alias: norm(bt), normalized: bt, type: 'body', is_active: true, priority: 70 }, { onConflict: 'alias' });
    }
  }
    ...
    try {
      ...
      if (data.success) {
        setDuzenleData((prev: any) => ({
          ...
          stops: data.result.stops?.map((s: any) => ({ id: null, city: s.city || 'İstanbul', district: s.district || '', weight_ton: s.weight_ton || '', pallet_count: s.pallet_count || '', vehicle_count: 1, cargo_type: s.cargo_type || '', notes: '' })) || prev.stops
        }));
    ...
    <div style={{ minHeight: '100vh', background: '#0d1117', fontFamily: "'IBM Plex Sans', system-ui, sans-serif" }}>
      ...
      <nav style={{ background: '#161b22', borderBottom: '1px solid #30363d', position: 'sticky', top: 0, zIndex: 50 }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 16px', height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button onClick={cikisYap} style={{ background: 'none', border: '1px solid #30363d', color: '#8b949e', borderRadius: 6, padding: '4px 12px', fontSize: '0.8rem', cursor: 'pointer' }}>
            Çıkış
          </button>
        </div>
      </nav>
          ...
          <div style={{ maxWidth: 1400, margin: '0 auto', padding: '10px 16px', display: 'flex', gap: 20, flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ display: 'flex', gap: 16 }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ color: '#fb923c', fontWeight: 800, fontSize: '1.4rem', lineHeight: 1 }}>{istatistik.pending ?? '—'}</div>
                <div style={{ color: '#6b7280', fontSize: '0.68rem', marginTop: 2 }}>Bekleyen</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ color: '#60a5fa', fontWeight: 800, fontSize: '1.4rem', lineHeight: 1 }}>{istatistik.bugunGelen ?? '—'}</div>
                <div style={{ color: '#6b7280', fontSize: '0.68rem', marginTop: 2 }}>Bugün Gelen</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ color: '#22c55e', fontWeight: 800, fontSize: '1.4rem', lineHeight: 1 }}>{istatistik.bugunOnaylanan ?? '—'}</div>
                <div style={{ color: '#6b7280', fontSize: '0.68rem', marginTop: 2 }}>Bugün Onaylanan</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ color: '#f87171', fontWeight: 800, fontSize: '1.4rem', lineHeight: 1 }}>{istatistik.cozumsuz ?? '—'}</div>
                <div style={{ color: '#6b7280', fontSize: '0.68rem', marginTop: 2 }}>Çözümsüz</div>
              </div>
            </div>

            {/* Renk hattı sayaçları — sadece pending */}
            {filtre === 'pending' && (
              <div style={{ display: 'flex', gap: 10, marginLeft: 8, borderLeft: '1px solid #1f2937', paddingLeft: 16 }}>
                <button onClick={() => setFiltreSkor(filtreSkor === 'yesil' ? 'hepsi' : 'yesil')}
                  style={{ background: filtreSkor === 'yesil' ? '#0d2b1a' : 'none', border: '1px solid', borderColor: filtreSkor === 'yesil' ? '#166534' : '#1f2937', borderRadius: 6, padding: '4px 12px', cursor: 'pointer', color: '#22c55e', fontSize: '0.8rem', fontWeight: 600 }}>
                  🟢 {yesil} Hazır
                </button>
                <button onClick={() => setFiltreSkor(filtreSkor === 'sari' ? 'hepsi' : 'sari')}
                  style={{ background: filtreSkor === 'sari' ? '#2a1d00' : 'none', border: '1px solid', borderColor: filtreSkor === 'sari' ? '#854d0e' : '#1f2937', borderRadius: 6, padding: '4px 12px', cursor: 'pointer', color: '#fbbf24', fontSize: '0.8rem', fontWeight: 600 }}>
                  🟡 {sari} Kontrol Et
                </button>
                <button onClick={() => setFiltreSkor(filtreSkor === 'kirmizi' ? 'hepsi' : 'kirmizi')}
                  style={{ background: filtreSkor === 'kirmizi' ? '#2a0d0d' : 'none', border: '1px solid', borderColor: filtreSkor === 'kirmizi' ? '#7f1d1d' : '#1f2937', borderRadius: 6, padding: '4px 12px', cursor: 'pointer', color: '#f87171', fontSize: '0.8rem', fontWeight: 600 }}>
                  🔴 {kirmizi} Düzenleme Gerek
                </button>
                {filtreSkor === 'yesil' && yesil > 0 && (
                  <button onClick={topluOnaylaYesil}
                    style={{ background: '#14532d', border: '1px solid #166534', borderRadius: 6, padding: '4px 14px', cursor: 'pointer', color: '#22c55e', fontSize: '0.8rem', fontWeight: 700 }}>
                    ⚡ Tümünü Onayla ({yesil})
                  </button>
                )}
              </div>
        ...
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '8px 16px' }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', marginBottom: 8 }}>
            {(['pending', 'approved', 'rejected', 'passive', 'hepsi', 'no_lane'] as const).map(f => (
              <button key={f} onClick={() => { setFiltre(f); setSonraBak(new Set()); setAramaMetni(''); setFiltreTelefon(''); setFiltreKalkis(''); setFiltreVaris(''); setFiltreAracTipi(''); setFiltreSkor('hepsi'); }}
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <div style={{ position: 'relative', flex: '1', minWidth: 180 }}>
              ...
              <input
                ...
                placeholder="Kelime ara (ham metin, not, şehir...)"
                style={{ ...inp, paddingLeft: 28, borderRadius: 6 }}
              />
            </div>
            ...
            <input
              value={filtreTelefon}
              onChange={e => setFiltreTelefon(e.target.value)}
              placeholder="📞 Telefon"
              style={{ ...inp, width: 130, borderRadius: 6 }}
            />

            {/* Kalkış ili */}
            <select value={filtreKalkis} onChange={e => setFiltreKalkis(e.target.value)}
              style={{ ...inp, width: 130, borderRadius: 6 }}>
            ...
            {/* Varış ili */}
            <select value={filtreVaris} onChange={e => setFiltreVaris(e.target.value)}
              style={{ ...inp, width: 130, borderRadius: 6 }}>
            ...
            {/* Araç tipi */}
            <select value={filtreAracTipi} onChange={e => setFiltreAracTipi(e.target.value)}
              style={{ ...inp, width: 120, borderRadius: 6 }}>
              <option value=''>🚛 Araç</option>
              {ARAC_TIPLERI.map(t => <option key={t}>{t}</option>)}
            </select>

            {/* Temizle */}
            {(aramaMetni || filtreTelefon || filtreKalkis || filtreVaris || filtreAracTipi || filtreSkor !== 'hepsi') && (
              <button onClick={() => { setAramaMetni(''); setFiltreTelefon(''); setFiltreKalkis(''); setFiltreVaris(''); setFiltreAracTipi(''); setFiltreSkor('hepsi'); }}
                style={{ background: 'none', border: '1px solid #374151', color: '#6b7280', borderRadius: 6, padding: '4px 10px', fontSize: '0.78rem', cursor: 'pointer' }}>
                ✕ Temizle
              </button>
            )}
          </div>
      ...
      <main style={{ maxWidth: 1400, margin: '0 auto', padding: '16px' }}>
        {filtre === 'no_lane' ? (
            ...
            {noLaneListesi.length === 0 ? (
              ...
              <div key={raw.id} style={{ background: '#161b22', border: '1px solid #30363d', borderRadius: 8, padding: '14px 16px' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button onClick={() => { setDuzenleData({ listing_type: 'yuk', origin_city: 'İstanbul', origin_district: '', contact_phone: '', price_offer: '', notes: raw.raw_text.split('\n')[0], vehicle_type: [], body_type: [], raw_post_id: raw.id, raw_text: raw.raw_text, stops: [{ id: null, city: 'İstanbul', district: '', weight_ton: '', pallet_count: '', vehicle_count: 1, cargo_type: '', notes: '' }] }); setDuzenleId('no_lane_' + raw.id); }}
                ...
                {duzenleId === 'no_lane_' + raw.id && (
                  <div style={{ marginTop: 12, borderTop: '1px solid #1f2937', paddingTop: 12 }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}>
                      <div><div style={{ color: '#8b949e', fontSize: '0.68rem', marginBottom: 2 }}>İlan Tipi</div>
                        <select value={duzenleData.listing_type} onChange={e => setDuzenleData({ ...duzenleData, listing_type: e.target.value })} style={inp}><option value="yuk">Yük</option><option value="arac">Araç</option></select></div>
                      <div><div style={{ color: '#8b949e', fontSize: '0.68rem', marginBottom: 2 }}>Telefon</div>
                        <input value={duzenleData.contact_phone} onChange={e => setDuzenleData({ ...duzenleData, contact_phone: e.target.value })} style={inp} /></div>
                      <div><div style={{ color: '#8b949e', fontSize: '0.68rem', marginBottom: 2 }}>Kalkış İli</div>
                        <select value={duzenleData.origin_city} onChange={e => setDuzenleData({ ...duzenleData, origin_city: e.target.value })} style={inp}>{ILLER.map(il => <option key={il}>{il}</option>)}</select></div>
                      <div><div style={{ color: '#8b949e', fontSize: '0.68rem', marginBottom: 2 }}>Kalkış İlçesi</div>
                        <input value={duzenleData.origin_district} onChange={e => setDuzenleData({ ...duzenleData, origin_district: e.target.value })} style={inp} placeholder="İlçe" /></div>
                    </div>
          ...
          <div style={{ textAlign: 'center', padding: '60px 0', color: '#4b5563' }}>
            ...
            {sonraBakSayisi > 0 && (
              <button onClick={() => setSonraBakGoster(true)} style={{ marginTop: 16, background: 'none', border: '1px solid #451a03', color: '#fb923c', borderRadius: 6, padding: '6px 16px', cursor: 'pointer', fontSize: '0.85rem' }}>
                      ...
                      {duzenleniyor ? (
                        <div>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}>
                            ...
                            <div><div style={{ color: '#8b949e', fontSize: '0.68rem', marginBottom: 2 }}>Kalkış İlçesi</div>
                              <input value={duzenleData.origin_district} onChange={e => setDuzenleData({ ...duzenleData, origin_district: e.target.value })} style={inp} placeholder="İlçe" /></div>
                            <div><div style={{ color: '#8b949e', fontSize: '0.68rem', marginBottom: 2 }}>Fiyat (TL)</div>
                              <input type="number" value={duzenleData.price_offer} onChange={e => setDuzenleData({ ...duzenleData, price_offer: e.target.value })} style={inp} placeholder="Opsiyonel" /></div>
                          </div>
                          <div style={{ marginBottom: 10 }}>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                              {ARAC_TIPLERI.map(tip => {
                                ...
                                return <button key={tip} onClick={() => setDuzenleData({ ...duzenleData, vehicle_type: secili ? [] : [tip] })}
                                  style={{ padding: '3px 10px', borderRadius: 4, border: '1px solid', borderColor: secili ? '#22c55e' : '#374151', background: secili ? '#14532d' : '#0d1117', color: secili ? '#22c55e' : '#9ca3af', fontSize: '0.78rem', cursor: 'pointer' }}>{tip}</button>;
                              })}
                            </div>
                          </div>
                          <div style={{ marginBottom: 10 }}>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                              {UST_YAPI.map(tip => {
                                ...
                                return <button key={tip} onClick={() => { const m = duzenleData.body_type || []; setDuzenleData({ ...duzenleData, body_type: secili ? m.filter((t: string) => t !== tip) : [...m, tip] }); }}
                                  style={{ padding: '3px 10px', borderRadius: 4, border: '1px solid', borderColor: secili ? '#60a5fa' : '#374151', background: secili ? '#1e3a5f' : '#0d1117', color: secili ? '#60a5fa' : '#9ca3af', fontSize: '0.78rem', cursor: 'pointer' }}>{tip}</button>;
                              })}
                            </div>
                          </div>
                          {duzenleData.stops?.map((stop: any, idx: number) => (
                          ...
                          <div><div style={{ color: '#8b949e', fontSize: '0.68rem', marginBottom: 2 }}>Not</div>
                            <textarea value={duzenleData.notes} onChange={e => setDuzenleData({ ...duzenleData, notes: e.target.value })} rows={2} style={{ ...inp, resize: 'vertical' }} placeholder="Genel not" /></div>
                          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                            <button onClick={stopEkle} style={{ padding: '5px 12px', borderRadius: 4, border: '1px solid #374151', background: '#0d1117', color: '#22c55e', fontSize: '0.78rem', cursor: 'pointer' }}>+ Varış Ekle</button>
                            <button onClick={() => llmSor(ilan)} disabled={llmYukleniyor} style={{ padding: '5px 12px', borderRadius: 4, border: '1px solid #374151', background: '#0d1117', color: '#a78bfa', fontSize: '0.78rem', cursor: 'pointer', opacity: llmYukleniyor ? 0.5 : 1 }}>
                              {llmYukleniyor ? '⏳...' : '🤖 LLM\'e Sor'}
                            </button>
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
                            ...
                            {(ilan.vehicle_type || []).map((v: string) => (
                              <span key={v} style={{ background: '#1a2535', color: '#60a5fa', fontSize: '0.72rem', padding: '1px 6px', borderRadius: 4 }}>🚛 {v}</span>
                            ))}
                            {(ilan.body_type || []).map((b: string) => (
                              <span key={b} style={{ background: '#1f2937', color: '#94a3b8', fontSize: '0.72rem', padding: '1px 6px', borderRadius: 4 }}>{b}</span>
                            ))}
