...
// 🗑️ `mukerrerBul()` 11 Ağu 2026'da KALDIRILDI. Yerini `lib/dedup.ts`'teki
// birleşik SHA-256 hash'i aldı: telefon + tam rota + araç/kasa + toplam ton/palet
// + tarih. Eski il/ilçe+tarih beşlisi Bayram'ın "her şeyiyle aynı" tanımına
// uymuyordu ve iki ayrı bug taşıyordu (completed_at ve il-yalnız karşılaştırma,
// bkz. git geçmişi + `docs/PROJE_HARITASI.md` §9). Yeni kontrol
// `lib/ilan-yaz.ts::ilanYaz()` içinde, `dedup_hash` kolonuna karşı tek sorgu.
