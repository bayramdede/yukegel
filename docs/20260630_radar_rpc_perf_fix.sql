-- ============================================================
-- Radar Intelligence RPC — Performans Düzeltmesi
-- Tarih: 2026-06-30
-- Sorun: route_listings CTE'sinde tarih filtresi yoktu →
--        tüm listings tablosu taranıyordu → statement timeout
-- Çözüm: 365 günlük cap eklendi. total_loads artık "son 1 yıl"
--        anlamına gelir (all-time değil). Mevcut index
--        (origin_city, created_at DESC) artık kullanılabilir.
-- ============================================================
...
DECLARE
  ...
  v_cap       timestamptz := now() - interval '365 days';   -- << YENİ: 1 yıllık cap
...
BEGIN
  ...
  WITH route_listings AS (
    ...
    FROM public.listings l
    WHERE
      l.created_at >= v_cap                          -- << YENİ: 1 yıllık cap (eski: tüm zamanlar)
      AND (v_from IS NULL OR l.origin_city ILIKE '%' || v_from || '%')
      AND (v_to IS NULL OR EXISTS (
        SELECT 1 FROM public.listing_stops ls
        WHERE ls.listing_id = l.id
