...
-- 4.3 🚨 TEK KAYNAK KONTROLÜ — locations.json ile tablo aynı mı
--     Bu sorgu SQL'den çalışmaz; `npm run` tarafında bir test olarak yazılmalı.
--     JSON'daki (il, ilçe) çiftlerini tablodakiyle karşılaştırır, fark varsa
--     kırılır. BÖLÜM 2'deki uyarının otomatik hâli — yoksa ayrışma canlıda
--     `district_official` yanlış cevap verene kadar fark edilmez.
--     ⚠️ Bu test HENÜZ YAZILMADI.
