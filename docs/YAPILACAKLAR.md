...
- [ ] **Alias homonim temizliği** — `araç`, `olur`, `pazar`, `perşembe`, `merkez` gibi ilçe adları günlük kelime olarak geçtiği için hâlâ sahte şehir üretiyor. Kod düzeltmesi bunu çözmez; `docs/20260728_alias_homonim_temizligi.sql` ADIM 1–2 çalıştırılıp pasifleştirilecek liste belirlenmeli. Hem gatekeeper'ı hem `parse-listing`'i etkiliyor.
