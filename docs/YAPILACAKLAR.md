...
- [ ] **⛔ BLOKE — `post_date` sadeleştirme**: Adım 0 doğrulaması 0 yerine **1453 ayrışmış satır** döndü (59.533 içinde, %2,4). İki kolon her zaman aynı değilmiş. Migration'a `ADIM 0b` teşhis sorguları eklendi; özellikle `(clean_hash, message_date)` kopya sorgusu satır döndürürse yeni unique indeks hiç kurulamaz. Teşhis bitmeden ADIM 1 çalıştırılmamalı. Tablo 59.533 satır / 54 MB → küçük, Seçenek A (CONCURRENTLY'siz) yeterli. — `docs/20260728_raw_posts_post_date_sadelestirme.sql`. SIRA ÖNEMLİ: (1) SQL adım 0–2, (2) koddan `post_date` yazımını kaldır + dağıt, (3) SQL adım 3 ile kolonu düşür. Sırayı bozmak tüm insert'leri kırar.
